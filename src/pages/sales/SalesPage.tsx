...
const SalesPage = () => {
  ...
  const {
    salesForm: { selectedItems, payments, discountAmount, freeSale },
        ...
        const updateItems = selectedItems.map((v) => {
          if (v?.id === existingItem?.id) {
            ...
            const priceAfterDiscount =
              v.sellPrice * (1 - (v.discount ?? 0) / 100);

            return {
              ...v,
              quantity: newQuantity,
              totalPrice: parseFloat(
                (priceAfterDiscount * newQuantity).toFixed(2)
              ),
            };
          } else {
            return v;
  ...
  useEffect(() => {
    if (discountAmount > 0) {
      ...
      dispatch(
        changeDueAmount(!freeSale ? totalPrice - totalPaid - discountAmount : 0)
      );
    } else {
      ...
      dispatch(
        changeDueAmount(
          !freeSale ? (totalAmount > 0 ? totalAmount - totalPaid : 0) : 0
        )
      );
  ...
  }, [selectedItems, payments, dispatch, freeSale, discountAmount]);
