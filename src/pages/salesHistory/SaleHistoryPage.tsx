              ...
              {isLoading || isFetching ? (
                ...
                SalesHistory.map((sale: ISaleHistory, index: number) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      {formatDateToLongDate(sale.createdAt)}
                    </td>
                    <td className="p-3">{sale.invoiceId}</td>
                    <td className="p-3">
                      {sale.customer ? sale.customer.name : "N/A"}
                    </td>
                    <td className="p-3">
                      {sale.isCorporateSale ? "Yes" : "No"}
                    </td>
                    <td className="p-3">
                      {sale.dueAmount >= 0 ? sale.dueAmount : 0}
                    </td>
          ...
          {!isLoading && (meta as IMetaInfo)?.totalPages > 1 && (
            <section className="flex items-center justify-between pt-4 mt-4 border-t border-gray-200">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-300">
                Total Sales : {(meta as IMetaInfo)?.total}
              </p>
              <Pagination
