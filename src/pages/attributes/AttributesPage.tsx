...
const AttributesPage = () => {
    ...
    const handleSave = async () => {
        ...
        const filterAttributes = attributes
            .filter(
                (attr) =>
                    attr.name.trim() !== "" && attr.attributeValues.length > 0
            )
    ...
    return (
        <div className="bg-white dark:bg-stone-700 shadow-sm">
            <h1 className="p-3 bg-[#F9F9F9] dark:bg-stone-500 text-sm border-b dark:text-gray-200 border-gray-200 dark:border-gray-500">
                Manage Attributes
            </h1>
            <div className="p-3 overflow-x-auto">
                <div className="flex py-2 items-center gap-4 border-b border-gray-200 dark:text-gray-300 text-sm">
                    <span className="w-[30%]">Name</span>
                    <span className="w-[60%]">Values</span>
                    <span className="w-[10%] shrink-0">Delete</span>
                </div>
                {isLoading ? (
                    <div className="text-grey-500 dark:text-gray-300 text-sm">
                        Loading Attributes...
                    </div>
                ) : !isLoading && attributes.length > 0 ? (
                    attributes.map((attribute, i) => (
                        <div
                            className="flex items-center py-2 gap-4 border-b dark:text-gray-300 border-gray-200 text-sm"
                            key={i}
                        >
                            <div className="w-[30%]">
                                <input
