...
const RecivingsPage = () => {
    ...
    useEffect(() => {
        if (discountAmount > 0) {
            ...
            const totalPaid = payments.reduce((sum, p) => sum + p.amount, 0);
            dispatch(
                changeDueAmount(totalAmount > 0 ? totalAmount - totalPaid : 0)
            );
            if (recieveVariant.length <= 0 && payments.length > 0) {
                dispatch(removePayments([]));
            } else if (recieveVariant.length <= 0) {
    ...
    return (
        <div className="flex flex-wrap lg:flex-nowrap items-start gap-4 dark:text-gray-300">
            <div className="w-full">
                <div className="bg-white dark:bg-stone-700 p-5 flex relative">
                    <Link
                        to={"/items/new-item"}
                    ...
                    <div className="bg-white dark:bg-stone-600 absolute top-full  left-0 w-full z-10 ">
                        {searchItemValue.length >= 3 &&
                            !isSuggestionsLoading &&
                            !isSuggestionsFetching &&
                            suggestions?.length > 0 && (
                                <ul className="border border-gray-200 dark:bg-stone-600 max-h-[300px] overflow-y-auto">
                                    {suggestions.map(
                                        (item: IProductSuggestions) => (
                                            <li
                                                key={item.id}
                                                onClick={() =>
                ...
                <div className="bg-white dark:bg-stone-700 pt-2 pb-10 mt-4 paper-cut relative">
                    <div className="overflow-x-auto">
                        <table className="w-full border-collapse rounded-md text-gray-700 dark:text-gray-300">
                            <thead>
                                <tr className="bg-[#F9FBFC] dark:bg-stone-500 border-t border-b border-gray-200 *:font-semibold text-sm">
