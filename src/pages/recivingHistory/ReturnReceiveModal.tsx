...
const ReturnReceiveModal: React.FC<ReturnSaleModalProps> = ({ receiveId }) => {
    const dispatch = useDispatch();

    const { data: selectedReceive, isLoading: dataLoading } =
        useGetReceiveHistoryByIdQuery(receiveId, { skip: !receiveId });
    ...
    const getMainStock = (id: string) => {
        const find = selectedReceive?.recieveVariant?.find(
            (i: IReceiveHistory) => i?.id === id
        );
        ...
        <div className="overflow-y-auto fixed top-0 left-0 w-full h-screen bg-black/20 flex justify-center items-start z-[999] p-4">
            ...
            >
                {dataLoading ? (
                    ...
                    <>
                        <h3 className="mb-5 text-lg font-medium dark:text-gray-300">
                            Return items
                        </h3>
                        <button
                            type="button"
                            className="absolute top-0 right-0 p-4 dark:text-gray-300 dark:hover:text-gray-400"
                        ...
                        {selectedReceives?.length > 0 && (
                            <div className="overflow-x-auto">
                                <table className="w-full border-collapse rounded-md text-gray-700 dark:text-gray-300">
                                    <thead>
                                        <tr className="bg-gray-100 dark:bg-stone-500 border-t border-b border-gray-200 border *:font-semibold text-sm">
                                            <th className="p-3">~</th>
                                                ...
                                                >
                                                    ...
                                                    <td className="p-3 flex items-center gap-3 justify-center">
                                                        <button
                                                            ...
                                                            className="p-1 px-2 text-sm border dark:text-gray-300 border-gray-600 text-black disabled:text-gray-200 disabled:border-gray-200 cursor-pointer"
                                                        >
                                                            -
                                                        </button>
                                                        {value.quantity}
                        ...
                        {removedItems.length > 0 ? (
                            <div className="overflow-x-auto mt-5">
                                ...
                                <table className="w-full border-collapse rounded-md text-gray-700 dark:text-gray-300">
                                    <thead>
                                        <tr className="bg-gray-100 dark:bg-stone-500 dark:text-gray-300 border-t border-b border-gray-200 border *:font-semibold text-sm">
                                            <th className="p-3">Item Name</th>
                                            <th className="p-3">Quantity</th>
