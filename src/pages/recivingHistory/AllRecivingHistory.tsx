            ...
            <tbody className="text-sm">
              {isLoading ? (
                ...
                receivingHistory?.map(
                  (receiving: IReceiveHistory, index: number) => (
                    <tr
                      onClick={() => {
                        setSaleRecivingDetails(receiving?.id);
                        setShowModal(true);
                      }}
                      key={index}
                    ...
                    >
                      ...
                      <td className="p-3">
                        {formatDateToLongDate(receiving.createdAt)}
                      </td>
                      <td className="p-3">{receiving.invoiceId}</td>

                      <td className="p-3">{receiving?.supplier?.fullName}</td>

                      <td className="p-3">
                        {receiving.dueAmount > 0 ? receiving.dueAmount : 0}
                      </td>
                      <td className="p-3">{receiving.paidAmount}</td>
                      <td className="p-3">{receiving.totalPrice}</td>
          ...
          {!isLoading && (meta as IMetaInfo)?.totalPages > 1 && (
            <section className="flex items-center justify-between pt-4 mt-4 border-t border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <p className="text-sm dark:text-gray-300">Show per page :</p>
                <select
